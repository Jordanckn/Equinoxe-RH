...
export function HomePage() {
  ...
  return (
    <>
      ...
      <section className="overflow-hidden bg-ivory border-b border-ink/5">
        <Container className="pt-16 pb-12">
          <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <p className="mb-5 text-sm font-semibold uppercase tracking-[0.18em] text-sage-dark">Toulouse, Occitanie et accompagnement à distance</p>
              <h1 className="font-serif text-5xl font-semibold leading-[1.05] text-ink sm:text-7xl">Conseil RH, coaching et accompagnement du changement</h1>
              <p className="mt-7 max-w-3xl text-lg leading-8 text-anthracite/80">Caroline Tillou Maratuech accompagne les entreprises, les dirigeants, les managers et les particuliers dans leurs transitions professionnelles, humaines et organisationnelles, avec une approche à la fois experte, académique et profondément humaine.</p>
            ...
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative w-full max-w-[440px] lg:max-w-full aspect-square rounded-2xl border border-ink/5 bg-white/40 p-4 shadow-soft">
                <img 
                  src="/images/hero_illustration.png" 
                  alt="Illustration Equinoxe Conseil RH - Équilibre et Transition" 
          ...
          <div className="mt-16 rounded-2xl border border-ink/10 bg-white/50 p-6 md:p-8 shadow-soft">
            <h2 className="font-serif text-xl font-semibold text-ink mb-6 text-center md:text-left">Une expertise hybride, académique et terrain :</h2>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                <div key={String(label)} className="flex gap-4 items-start border-r border-ink/10 last:border-0 pr-4 last:pr-0 md:border-r-0 md:border-b md:pb-4 md:last:border-0 md:last:pb-0 lg:border-r lg:border-b-0 lg:pb-0">
                  <span className="rounded-full bg-sage/20 p-2.5 text-sage-dark mt-0.5"><Icon size={18} /></span>
                  <div>
                    <h4 className="font-semibold text-ink leading-tight text-sm sm:text-base">{String(label)}</h4>
        ...
        <Container className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div key={item} className="flex items-center gap-4 rounded-xl border border-ink/5 bg-ivory/40 p-5 shadow-sm hover:border-sage-dark/20 hover:bg-white hover:shadow-md transition-all duration-300">
                <span className="rounded-full bg-sage/20 p-2 text-sage-dark">
      ...
      <Section className="bg-ink text-white">
        <Container className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          ...
          <Link className="focus-ring rounded-full bg-white px-6 py-3 font-semibold text-ink hover:bg-champagne hover:text-white transition-colors duration-300" to="/contact">Parler de votre situation</Link>
        </Container>
      </Section>
    </>
  );
...
function GridTitle({ title, text }: { title: string; text: string }) {
  return <div><h2 className="font-serif text-4xl font-semibold text-ink sm:text-5xl">{title}</h2><p className="mt-4 max-w-2xl text-lg leading-8 text-anthracite/75">{text}</p></div>;
}
